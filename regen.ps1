...
$wt = 'C:\Users\SAMPC\lifesystemos-us-astro'
...
# ---------------------------------------------------------------
# 0. Stage public assets
# ---------------------------------------------------------------
$publicDir = "$wt\public"
New-Item -ItemType Directory -Force -Path $publicDir | Out-Null
foreach ($src in @("$wt\crown-design-system.css", "$wt\assets", "$wt\og-images")) {
  if (Test-Path $src) { Copy-Item $src $publicDir -Recurse -Force; Write-Host "staged: $src" }
}
if (Test-Path "$wt\CNAME") { Copy-Item "$wt\CNAME" $publicDir -Force; Write-Host "staged: CNAME" }

# ---------------------------------------------------------------
# 1. Remove old category-dir stub pages
# ---------------------------------------------------------------
$oldDirs = Get-ChildItem $pagesDir -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -notin @('index') }
foreach ($d in $oldDirs) { Remove-Item $d.FullName -Recurse -Force; Write-Host "removed old stub dir: $($d.Name)" }
...
# ---------------------------------------------------------------
$skipPages = @('index', '404', 'about', 'all-articles', 'contact', 'privacy', 'disclaimer', 'checklist', 'store', 'thank-you', 'checkout', 'products', 'terms')
$tpl = @'
---
...
const jsonLd = {
  ...
  publisher: { '@type': 'Organization', name: 'LifeSystemOS' },
};
---
...
foreach ($slug in $skipPages) {
  ...
  $m = $meta[$slug]
  if (-not $m) { $m = @{ title = $slug; desc = ''; canonical = "https://lifesystemos.com/$slug" } }
