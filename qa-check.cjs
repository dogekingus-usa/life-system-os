...
check('title', /<title>[^<]*LifeSystemOS/i.test(home), (home.match(/<title>([^<]*)<\/title>/) || [])[1]);
...
check('theme-lifesystemos', home.includes('class="theme-lifesystemos"'));
check('canonical', /rel="canonical" href="https:\/\/lifesystemos\.com\/?"/.test(home), (home.match(/rel="canonical" href="([^"]+)"/) || [])[1]);

console.log('=== ARTICLE PAGE ===');
const art = fs.readFileSync(path.join(dist, '10-minimalist-habits-that-free-up-hours-of-your-day.html'), 'utf8');
check('article title', /<title>[^<]*Minimalist Habits/i.test(art), (art.match(/<title>([^<]*)<\/title>/) || [])[1]);
check('h1 present', art.includes('<h1>') && /<h1>[^<]{20,}/.test(art));
check('FULL content (not stub)', (art.match(/<p>/g) || []).length > 3, `${(art.match(/<p>/g) || []).length} paragraphs`);
check('no [object Object]', !art.includes('[object Object]'));
check('no FFFD', !art.includes('\uFFFD'));
check('canonical flat', art.includes('rel="canonical" href="https://lifesystemos.com/10-minimalist-habits-that-free-up-hours-of-your-day"'));
